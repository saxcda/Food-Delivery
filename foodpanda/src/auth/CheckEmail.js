...
const CheckEmail = ({ email, onBack, onClose }) => {
  ...
  return (
    <Box className="checkEmailWrapper" display="flex" flexDirection="column">
      <Box
        className="checkEmailHeader"
        display="flex"
        justifyContent="space-between"
        alignItems="center"
      >
        <IconButton className="checkEmailBackButton" onClick={onBack}>
          <ArrowBackIcon />
        </IconButton>
        <IconButton className="checkEmailCloseButton" onClick={onClose}>
          <CloseIcon />
        </IconButton>
      </Box>

      <Box display="flex" flexDirection="column">
        <Box className="checkEmailIcon" />
        <svg
      ...
      <Typography variant="h5" className="checkEmailModalTitle">
        查看 email
      </Typography>
      <Typography variant="body1" className="checkEmailDescription">
        我們已將一個秘密連結發送到
      </Typography>
      <Typography variant="body2" className="checkEmailAddress">
        {email}。趕緊使用連結登入吧。
      </Typography>

      <Button
        className="checkEmailPrimaryButton"
        variant="contained"
        onClick={onClose}
      >
        我沒有收到 email
